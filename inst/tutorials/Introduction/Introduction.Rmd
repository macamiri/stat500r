...
description: >
  Learn how to use R to apply the concepts taught in a Statistics course.
---
...
are inspired by [Penn State's STAT 500: Applied Statistics course](https://online.stat.psu.edu/stat500/). 

This **tutorial assumes** you have **(1)** R/RStudio installed, **(2)** basic understanding of R (eg., how to load packages), and **(3)** sufficient knowledge of the material presented in STAT 500. This tutorial is suited for anyone who is interested in applying the concepts from a standard statistics course in R (we do not re-explain the concepts here).

Each section in this tutorial is associated with the same numbered lesson from the STAT 500 course. As you make your way, you will come across *code examples* and a few *multiple choice questions*. 
...
that you can run by clicking on the blue **Run Code** button in the upper right. In the event that you update the code and get an error message, just click on **Start Over** in the upper left corner to automatically reset the code to the default.

This code example calculates the mean for 1, 2, 3, 4 and 5. Try running the code below (output should be 3(. Then, change the 5 to a 4 and run the code again (new output is 2.5).
...
Here is a simple example. Submit your answer:
...
We are finally ready to start with Lesson 1 and statistically explore data in R! 

## Lesson 1

For our first lesson, we'll (1) learn about several R functions that are used to summarize data, and (2) start getting familiar with the **tidyverse**.

### `r fa("fas fa-lightbulb")` Code 1.1: Sample

Let's take a sample of 5 customer names without replacement from our grocery data frame. We can do so by using [`sample()`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/sample). Press 'Run Code' to sample. You will get a new sample each time you run the code.

\**Note: To get a reproducible outcome (the same outcome every time you run the code), simply use the `set.seed()` function before sampling. `set.seed()` somewhat controls for the 'randomness' of the random part of the sampling.*
...
\**Note: To learn more about what each function does, write `?` before the function name. For example, run `?mutate` in your R/RStudio environment.*
...
It follows a simple idea of building visualization layers on top of each other with the `+` operator as you will see below. This means that the order in which you add the functions/layers matters when building plots using **ggplot2**.

Before building our pie chart, we will need to do some data prep. We need to count the absolute (#) and relative (%) frequency of each payment method.

Notice the assignment ` <- ` operator. It is used to save (i.e., assign) the results of some piece of code to some variable. Let's see everything in action below. Remember that you do not need to memorize the functions. Whenever you need help, just type `?*function name*` to remind yourself of the functions.
...
# data prep - count frequency of each payment method
grocery_pie <- 
  grocery_table %>% 
  count(payment_method, name = "freq") %>% 
  mutate(prop = scales::percent(freq / sum(freq)))

# draw pie chart using geom_col and coord_polar
grocery_pie %>% 
...
Do you recall that **ggplot2** builds visualization layers on top of each other? In the code example above, move `theme_void()` to the end and notice how the plot changes Keep in mind that you do not need to memorize in what order to write the functions; with more practice you'll have a better understanding of what each layer does.

### `r fa("fas fa-lightbulb")` Code 1.4: Bar Chart

Next up is the bar chart. Let's summarize the same data from Code 1.3 in the form of a bar chart. If you followed the pie chart example, then the bar chart will be a breeze. The `geom_bar()` function is what we need to build the bar chart.
...
To find the mean and median in R is super easy, simply use `mean()` and `median()`. Meanwhile, base R does not have a function for mode, so we will need to create our own function to compute the mode. This will be worth seeing if you've never defined a function before.

Let's practice using **dplyr** with the pipe ` %>% ` operator by computing the measures of central tendency for the *cost* variable. First, we will define our mode function.

Before we do that, do you know what a *function* is? It has a similar meaning to functions in Mathematics. You input some values into the arguments (think of them as variables), then the function produces an output. That's it.

The `function()` command lets R know that you are about to define a function. The arguments/inputs of the function are inserted between the `()`. Finally, the body of the function goes inside the curly brackets `{}`. An example will make things clear. Let's start by building a basic sum function, followed by a mode function.
...
We called our function *addition* and will need to type it followed by *()* every time we want to invoke it. The arguments first_num and second_num are the variables where we include our inputs. Change the numbers 1 and 3 and re-run the code.

Now, let's define a mode function and summarise the *cost* variable.
...
Alternatively, you can use the `Mode()` function in the `DescTools` package.

### `r fa("fas fa-lightbulb")` Code 1.6: Position

To find any percentile, simply use `quantile()` Let's find the 20, 25, 50, 75 and 99th percentile of the *cost* variable.

```{r quantiles, exercise = TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
quantile(grocery_table$cost, probs = c(.2, .25, .5, .75, .99))
```

Notice that the 50th percentile = median from Code 1.5.
...
Let's create a dotplot using **ggplot2**. Let's take this opportunity to learn another useful function `filter()`. We will draw a dotplot to show the distribution of customer age only for households of size 3.
...
grocery_table %>% 
  ...
  viridis::scale_color_viridis(discrete = TRUE, end = .85) + 
  labs(y = "Price") + 
  scale_x_continuous(breaks = NULL) + 
  theme(legend.position = "none")
```

This is the end of our first lesson!

## Lesson 2

In this section, we will cover Venn diagrams and probability network/tree graphs.

### `r fa("fas fa-lightbulb")` Code 2.1: Ice Cream Data

Think of a *list* object in R as the general form of a table: 2 differences between a *list* and table are (1) the elements in the *list* do not have to be the same length (vs columns in a table), and (2) a table is 2-dimensional, while a *list* can be n-dimensional.

We will create a *list* object to track what flavors were bought across 3 days at an ice cream shop. Assume that (1) each flavor can be bought only once a day (sample without replacement), (2) all the flavors are restocked at the end of the day, and (3) they are equally likely to be chosen.

Note our use of `sample()` and `set.seed()` (see Code 1.1).
...
How many of the 50 possible flavors were bought on all 3 days? What about those that were chosen only on day 1 and day 2? Let's build a venn diagram to easily identify the answers.
...
The venn diagram above is telling us that 7 out of the 50 flavors were bought on both day 1 and 2, but not on day 3. None of the flavors (0) were bought on all 3 days.
...
```{r venn-quiz}
quiz(
  question("How many flavors were only bought on day 3?",
    answer("0"),
    answer("2"),
...
Imagine that you are flipping a magical coin twice. The coin is magical because the first flip is fair, while the second flip is biased (not 50/50). Everytime you get Tails on the first flip, the chance of getting a Tails on the second flip increases to 80%. The second flip remains fair (50/50) when the first flip results in Heads. Let's represent these probabilities as a tree diagram.
...
**(2)** P(2nd Tails | 1st Tails) = Only look at the top half of the graph since we know the first flip was Tails = `r .8`
...
quiz(
  ...
  question("Find P(2nd Tails).",
    answer("0.25"),
    answer("0.50"),
    answer("0.65", correct = TRUE, message = ".5(.8) + .5(.5)"),
    answer("0.80")
  ),
...
This brings us to the end of lesson 2. Catch you in lesson 3 & 4.
...
Notice that the shape of the histogram, in this case of data points randomly sampled from a uniform distribution, approaches the normal distribution as the sample size `n` increases from 10 -> 30 -> 100 -> 10,000. This is what the Central Limit Theorem is about.

The same simulation can be repeated by randomly sampling data points from different distributions. To do so in the code above, replace the `runif` function with the random generator function of another distribution. Here are some of the distributions:
...
Let's start with the Binomial distrbution to answer the 3 following questions. Assume the data satisfies the 4 conditions for the Binomial:
...
The output of `3` means that **the probability of getting either 0, 1, 2 or 3 Tails when flipping the coin 10 times** is *17%*. There's an *83%* (1 - .17) **probability of getting at least 4 Tails on 10 coin flips**. The same idea follows for the 37th quantile.

### `r fa("fas fa-lightbulb")` Code 3.3: Sampling Distribution of the Sample Mean (Normal Population)

The Normal distribution is the cornerstone of many statistical testing methods, so it is a good idea to become familiar with the functions for the Normal distribution in R. First, let's tackle the problems where the population we sample from is Normal.

This is a fictional example: the distribution of the weight of Australian sheep is normal with mean of 70 kg and standard deviation of 8.3 kg. Assume this follows a normal distribution. Now, let's use R to answer 3 common problems that you might come across in quizzes.
...
Now, assume that there is no indication that the sheep population from the Code 3.3 is Normal. Can we answer the same 3 questions? If you have read the notes, you know that our method depends on knowing that the distribution is Normal and in this section we do not make such an assumption.

Let's start by taking a random sample of size n = 166 sheeps from the population. By the Central Limit Theorem, we can now assume that the sampling distribution of the sample mean weight of the sheep is Normal with a mean of 70 kg and standard error of 8.3 / sqrt(166) = `r 8.3 / sqrt(166)`
...
Let's represent both distributions from sections 3.3 and 3.4 graphically using *ggplot2*:
...
Notice how the sampling distribution (CLT) is much narrower than the original normal population (Normal).
...
1) If a random sample of 40 restaurants were selected, what is the probability that the proportion of the sample that has a vegan meal on their menu is at least 70%? Assume that the conditions for the sampling distribution of the sample proportion are satisfied.
...
In the next section, be ready to explore confidence intervals.

## Lesson 5

It's time to start talking about statistical inference. In particular, we will be dicussing confidence intervals in R. Remember the general form of a confidence interval: `r cat(crayon::bold("CI = Point Estimate ± Margin of Error"))`.

### `r fa("fas fa-lightbulb")` Code 5.1: Confidence Intervals for Population Proportion (Manual)

The 'Manual' part means that we will be calculating each part of the confidence interval separately rather than using a specific function (see Code 5.3). Practically, you are better off using a function to calculate the interval for you, but here we will go about it the way it is presented in the course to strengthen your understanding of the concept.

Let's construct the confidence interval for the following: a random sample of 250 current NBA players and coaches is taken. They are asked whether they think Michael Jordan is the greatest basketball player of all time (i.e., GOAT rating). Of the 250 surveyed, 161 respond with 'yes'. Calculate a 95% confidence interval for the overall GOAT rating of Michael Jordan (assume that the conditions are satisfied).
...
We are 95% confident that between 58.46% and 70.34% (more than half) of current NBA players and coaches think Michael Jordan is the greatest basketball player of all time.
...
Since the sample size is 40 > 30, by the Central Limit Theorem, the sampling distribution of the sample mean departure delay time is approximately normal. We can use the *Student t distribution* to construct the 90% confidence interval:
...
We are 95% confident that the mean departure delay at the Los Angeles International Airport is between 19.14 and 22.86 minutes.

### `r fa("fas fa-lightbulb")` Code 5.3: Confidence Intervals (`Rmisc` package)

The `CI()` function in the `Rmisc` package constructs the confidence interval in one step, as long as we have the original data. We will quickly generate normal random data with mean of 21 and standard deviation of 7 (similar to the data from Code 5.2):
...
Notice that the confidence interval is similar to the one we constructed manually in Code 5.2. It is not exactly the same since the mean and standard deviations will not be exactly 21 and 7, respectively, when using `rnorm()`, but close enough for our purpose.

### `r fa("fas fa-lightbulb")` Code 5.4: Checking Normality

Finally, let's see how we can draw the normal probability plot in R to check if the data is normal. We will learn about 2 methods; the second allows for more customization.
...
r_data %>% 
  ...
  stat_qq_line(size = 1, color = "orange")
```
...
It is clear from the 3 plots above that data from the first and third code blocks are normal as the data values fall within the bounds. If we increase the confidence interval to .99, then the confidence bounds will be wider

This ends our discussion on confidence intervals. Next up is hypothesis testing.

## Lesson 6

In this section, we will explore hypothesis testing in R by conducting tests and drawing graphs to visually highlight what is going on. Be sure to read the course notes from lessons 6a and 6b before moving on with this section. In all the examples below, we assume that the necessary conditions are satisfied.
...
\**Note: To find the z-test statistic, you will need to take the square root of the X-squared statistic that the `prop.test()` function returns. The explanation for this relationship between the two statistics is beyond the scope of this tutorial*

We can also use `prop.test()` to calculate the confidence intervals. Take the same basketball example from Code 5.1 in lesson 5. We found that the 95% confidence interval to be (.5846, .7034). Let's use `prop.test()` to see if we get the same results (spoiler alert: we will).
...
# cereal box weights
set.seed(500)
cereal <- rnorm(200, mean = 29.4, sd = .2)
...
The confidence interval is (29.35, 29.41), indicating that 30 does not fall within the bounds, so we can reject the manufacturer's claim.

Before moving on to the next section, try running the hypothesis test examples from the course notes in R using the same methods as above.

## Lesson 7

Let's move on to one dimension higher from our previous lesson and compare two population parameters.
...
Suppose we asked high-school and university students wether they owned an Apple product or not (e.g., iphone, macbook laptop, Apple watch). Of the 166 high school students sampled, 108 said 'yes' and of the 315 university students sampled, 246 said 'yes'.

Does the data indicate that the proportions that said 'yes' (i.e., own an Apple product) are different for high-school and university students at a 5% significance level?
...
Take some time to familiarize yourself with the layout of the output.
...
Say we measured the resting heart rate (RHR) of 53 professional soccer players and 55 golfers. It is claimed that the soccer players have a lower RHR than golf athletes. Our measurements are as follows: mean_soccer = 59 beats per minute (bpm) and sd_soccer = 4.6 bpm, while mean_golf = 68 bpm and sd_golf = 6.4 bpm.
...
The `equal_var` object will returm `TRUE` if the variances can be assumed to be equal, which will in turn run the pooled variance t-test. If one of the variances is greater by a factor of 2 or more than the other variance, then `equal_var` object will returm `FALSE`, which tells `t.test()` to run the unpooled variance t-test.
...
# conduct 1 sample mean t-test
t.test(x = before, y = after, alternative = "greater", conf.level = .99, paired = TRUE)
```

To run a paired t-test in R, you have to set the `paired` argument in `t.test()` to `TRUE`.

Since p-value ~ 0 < .01, we can conclude that the workout routine can help individuals lose weight in as little as 4 weeks. Furthermore, we are 99% confident that the true mean weight loss is between 4.07 and 7.40 kg.

### `r fa("fas fa-lightbulb")` Code 7.4: Comparing Two Population Variances

Suppose that the statisticians working at a chocolate factory suspect that one of the older machines is no longer producing the desired weight of chocolate in each package consistently. They test their claim by comparing the variance in weight of 100 pieces of chocolate produced by the older machine and a newer one that they know works as desired.
...
# cereal box weights
set.seed(500)
old <- runif(n = 100, min = 2.43, max = 2.49)
...
Since p-value ~ 0 < .01, we can conclude that the old machine has a different variance than the newer well-working machine -> the factory will need to fix the old machine in order to produce the desired weight of chocolate consistently.

This concludes our section on comparing two population proportions, means and variances. Next, we will discuss how to use the Chi-Square test when both the response and the explanatory variable are qualitative.

## Lesson 8

How can we test if two categorical variables are dependent? Enter the Chi-Square Test of Independence. But first, let's take a look at contingency tables in R.
...
Let's start by creating a table for the 2 categorical variables **payment_method** (3 levels) and **household_size** (7 levels) from our **grocery_table** data (see Lesson 0). The contingency table will be 3 by 7.
...
# OR table(grocery_table$payment_method, grocery_table$household_size)
```

Avoid retyping the data name by using `with()`: the first argument takes the data, while the second argument takes the expression to evaluate.

We can add the total margins to the contingency table with `add_margins()`.
...
Next, let's visualize the relationship between **payment_method** and **household_size** with 3 barplots and a mosaic plot.
...
We can represent the same information, but with the bars placed next to each other. Simply change the argument for `position` from `"stack"` to `"dodge"` in `geom_bar()`.
...
What if you want to group the bars by each payment method? This is where `facet_wrap()` will help us acheive that goal by creating a seperate panel for each level in the grouping variable (i.e., payment method).

Again, it's clear that online payment is the most popular method. We can also note that households with 2 individuals are ordering the most.
...
What can a mosaic plot tell us? Let's create one below and find out:

```{r cont-table-six, exercise = TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
# load ggmosaic to use geom_mosaic()
library(ggmosaic)
...
Each 'block' of the mosaic plot tells us 2 pieces of info: (1) the vertically longer the block, the more orders used the payment method on the left, and (2) the horizontally wider the block, the more common the household size on the bottom. In this case, online payment was the longest block for every household size, reflecting that it was the most popular method across all household sizes. Also, households of size 7 had the narrowest blocks horizontally, reflecting that the fewest orders were placed by households of size 7.

The 4 graphs above more or less tell the same story, and they can all be further customized to suit your needs. Practically, you will start an anlysis by generating the simplest graph that can give you a basic idea of the data that you will be working with. Then, as your analysis develops, you can start to apply the customizations. Be mindful of the time that you spend customizing your graphs as it is very easy to fall into the rabbit hole of 'over-customization`. 

### `r fa("fas fa-lightbulb")` Code 8.2: Chi-Square Test

Can you tell if **household size** and **payment method** are dependent? Since both variables are categorical, we can run a chi-square test. Here's how to do it in R:
...
Since p-value = .2248 > .05, we can conclude that the variables are independent. This makes sense since our graphs from Code 8.1 showed that online payment was the most popular peyment method across all household sizes, followed by credit card on delivery.

To extract a specific part of an object in R, use the `$` symbol after the object's name:
...
# print all the items that you can call with '$'
chi_output %>% 
  map(~ print(.))
...
In this penultimate lesson we will explore correlation and simple linear regression. Linear regression is a topic that deserves its own tutorial, so we will only cover the absolute basics here to get you up and running.

When the response and explanatory variables are both quantitative, a good place to start is to analyze the linear relationship between them.
...
There's a lot going on in the diagram above, but if you read the course notes for lesson 9, then you will be able to interpret the output, which displays the scatter plot and correlation coefficient between each pair of variables, as well as the density distribution of each variable.
...
The output displays the correlation coefficients and the p-values for the correlation tests.
...
# load packages to use ggscatterstats()
library(ggstatsplot)
library(ggside)

# plot
mtcars %>% 
  ggscatterstats(x = mpg, y = hp, results.subtitle = TRUE) + 
  labs(caption = NULL)
```

Can we combine the two ideas for many variables at once, as we did with the correlation tests in Code 9.2? Absolutely! We can do that with `correlation()` from the *correlation* package:

```{r cor-test-four, exercise = TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
# load to use correlation()
library(correlation)

# correlation matrix
mtcars %>% 
  select(mpg, hp, disp, drat, wt, qsec) %>% 
  correlation(include_factors = TRUE, ci = .90)
```

The output above displays the correlation coefficient, 90% confidence interval (for the coefficient), t-statistic and adjusted p-value for all the pairwise correlation tests. With a few lines of code, we managed to get an idea of the linear relationships between the variables.

### `r fa("fas fa-lightbulb")` Code 9.4: Simple Linear Regression

We have finally reached the part where we build an SLR model and create some nice plots.
...
Each one of the code examples below will build on the previous ones, starting with this:
...
`str_glue()` is one of my all time favourite functions. Its power lies in its ability to conveniently evalute and integrate R expressions into a string. In other words, you can write R code within the curly braces `{}` and `str_glue()` will evaluate it and include it as part of the string.

Can we trust the resutls from `lm()`? We need to draw the diagnostic plots to check if the SLR conditions are satisfied. The basic way of doing this is via `plot()`.
...
Alternatively, we can qucikly create diagnostic plots with `autoplot()` after loading *ggfortify*. You can customize the layout of the graphs largely with the `theme()` function.
...
autoplot(model, which = 1:6, ncol = 3, color = "steelblue", label.repel = TRUE) + 
  theme_bw() + 
  theme(text = element_text(family = "Indie Flower")) # changes the title + axis labels to the 'Indie Flower' font
```

Here's how to interpret the plots:
A- Normal Q-Q to assess the normality condition. Notice the 4 points at the top right that are way above the line. This indictaes that the shape might be slgihtly right skewed. To further analyze the normality, we can plot the density of the residuals `plot(density(residuals(model)), main = "Density")`. In our case, we cannot confidently assume that the normality assumption is satisfied.
B- Residuals vs Fitted can help us to study the independence and variance of the errors. The points are spread out fairly evenly across the horizontal y = 0 line (i.e., there is no clear bowtie or megaphone shape) and there is no pattern, so both assumptions are not violated. We can run a formal test with the following: `lmtest::bptest(model)`.  
C- The Cook's distance and residuals vs leverage graphs help detect the potential outliers in the data (the 'how' is beyond the scope of this tutorial).  

We have reached the end of lesson 9. One more lesson to go.

## Lesson 10

We're almost across the finish line! Our final lesson is about ANOVA, one of the most important statistical methods to have in your toolbox. As we did with SLR, we will learn how to conduct one way anova and check for the conditions. We will end by running a post-hoc test.
...
Before running ANOVA, let's visualize the data with a boxplot and overlay the points with `geom_quasirandom()`:
...
At a quick glance, we can see that the products in **Grandiose Hypermarket Abu Dhabi** are the most expensive on average, but is the difference big enough to conclude a statistical significance? Let's find out.
...
Secondly, is the normality assumption satisfied? Let's plot the residuals to check. By drawing a histogram for the model's residuals, we can see that the shape is right tailed. However, since (1) ANOVA is fairly robust against violations of the normality assumption and (2) the sample size for each store/group is very large ---> we might be able to assume that the normal assumption holds. Further analysis is suggested before assuming so, but for the sake of this example, we will say that normality is satisfied.

In a real life setting, it might be better to also conduct a non-parametric test such as a Kruskal-Wallis Test (does not require the normality assumption) and compare the results. Alternatively, we can transform the response variable (price) to achieve a more normal distribution. For our case, we will assume that the normality assumption is satisfied.
...
Finally, we check if the variances are equal across the stores or not. A boxplot is a great quick way to check for homogeneity of variances Also, we will run a formal test using `leveneTest()`. Levene's test was briefly mentioned in lesson 7 of the course notes.
...
The good news is that we can use the same approach as we did in lesson 9 to create the diagnostic plots (see end of lesson 9 for the plot explanations):

```{r slr-six, exercise = TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
# anova
model_aov <- aov(price ~ store_name, data = grandiose)
...
autoplot(model_aov, which = 1:6, ncol = 3, color = "steelblue", label.repel = TRUE) + 
  theme_bw() + 
  theme(text = element_text(family = "Indie Flower")) # changes the title + axis labels to the 'Indie Flower' font
```

Recall that the `leveneTest()` above told us to assume heteroskedasticity of the residuals. This can also be seen in the residuals vs fitted values plot. The points get further apart as we go from left to right (i.e., megaphone shape).

### `r fa("fas fa-lightbulb")` Code 10.3: Post-hoc Tests

Remeber that our ANOVA returned a significant result (see Code 10.1). How can we see which groups are different from each other? Post-hoc tests allow us to run multiple comparisons for all pairwise means:
...
Let's rename the 3 stores so as to save space when we plot the confident intervals soon. We can do this with `mutate()` and `fct_recode()`.
...
# rename the levels to save space on the plot
grandiose2 <- 
  grandiose %>% 
  mutate(store_name = fct_recode(store_name, 
                                 Barsha = "Grandiose - Barsha", 
...
# anova
model_aov <- aov(price ~ store_name, data = grandiose2)

# tukey post-hoc test
TukeyHSD(model_aov, "store_name", ordered = TRUE, conf.level = 0.95)
```

The output indicates that all stores are significantly different in terms of mean product price (since adjusted p-value is ~ 0 for all pairwise comparisons).

Let's visualize the confident intervals for the differences:

```{r anova-eight, exercise = TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
# anova
model_aov <- aov(price ~ store_name, data = grandiose2)
...
It's super easy to fall into the trap of trying to customize every single detail in your graphs. In this section, we will quickly present the `ggstatplot` package to create powerful graphs that pack a lot of info with only a few lines of code. It is worth your time to read the [documentation on `ggstatplot`](https://indrajeetpatil.github.io/ggstatsplot/) if you will be running a lot of statistical tests in R.

We used `ggstatplot` once before in Code 9.3. Let's start by revisiting that example.

### `r fa("fas fa-lightbulb")` Code B.1: `ggscatterstats()` (from lesson 9)

```{r stat-one, exercise = TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
# load packages to use ggscatterstats()
library(ggstatsplot)
library(ggside)
...
mtcars %>% 
  ggscatterstats(x = mpg, y = hp, 
                 title = "Linear Relationship between HP and MPG", results.subtitle = TRUE) + 
  labs(caption = NULL)
```

We have graphed a scatter plot of **hp** vs **mpg** overlayed with the regression line and standard error bounds, a histrogram for each variable, and also included the results from the statistical test at the top. All of this in about 4 lines of code.

### `r fa("fas fa-lightbulb")` Code B.2: `ggbarstats()` (from lesson 8)

Let’s investigate whether **payment method** and **household size** are dependent The test checks whether the proportion of payment methods was different across the household sizes.

```{r stat-two, exercise = TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
# bar plot
grocery_table %>% 
  ggbarstats(x = payment_method, y = household_size, conf.level = .9) + 
  labs(caption = NULL) + 
  scale_fill_viridis_d() + 
  theme(legend.position = "bottom")
```

The graph shows that payment method proportions were not different across household sizez. The p-value .22 > .1, so we cannot say that the 2 variables are dependent. The p-values above each bar comes from the Goodness of Fit test for each household size.

### `r fa("fas fa-lightbulb")` Code B.3: `statsExpressions` Package

Finally, let's visit the `statsExpressions` package, which allows us to add statistical expressions to our plots.

Let's (1) build an ANOVA model to represent the relationship between **cost** and **payment method** from the **grocery_table** dataset. Then, we will (2) plot the relationship using a violin plot. Finally, we will (3) add the statistical expressions from the ANOVA model to the plot (this is done via `model_name$expression[[1]]`). That's a lot, but let's give it a try:

res <- oneway_anova(iris, Species, Sepal.Length, type = "nonparametric")

```{r stat-three, exercise = TRUE, exercise.eval = FALSE, exercise.startover = TRUE}
# load package
library(statsExpressions)

# anova - mean basket cost across the 3 payment methods
res_aov <- oneway_anova(data = grocery_table, x = payment_method, y = cost) # step (1)

# graph 
grocery_table %>% 
  ggplot(aes(x = payment_method, y = cost, color = payment_method)) + 
  geom_violin(trim = TRUE, fill = NA) + # step (2)
  geom_boxplot(width = .1) + 
  ggbeeswarm::geom_quasirandom(alpha = .1, size = 2) + 
  labs(x = "Payment Method", y = "Cost", 
       title = "Heteroscedastic 1-way ANOVA for Mean Order Value", 
       subtitle = res_aov$expression[[1]]) + # step (3)
  hrbrthemes::theme_ipsum(grid = FALSE) + 
  theme(legend.position = "none")
```

The main thing to note is that `res_aov$expression[[1]]` added the statistical expressions from the ANOVA output.

That's all I wanted to add to this lesson for anyone who wants to embed statistical expressions into plots. 

## Lesson 00: Final Thoughts

Congratulations! You have made it to the end of this tutorial and course. At this stage, you are able to start your own analysis from scratch and create publication worthy visualizations with *ggplot2*. Remember that you do not need to memorize all the functions presented in this tutorial. With time, you will be automatically become more familiar with the main functions. Focus on understanding the statistical theory first, and then you can search online on how to apply the concepts in R.
...
If you notice any theory/code mistakes in this tutorial, kindly let me know via Github:

- [Github Profile](https://github.com/moamiristat)

Hope you enjoyed this tutorial and good luck in your R journey! Remember that you can always visit this tutorial by running the following command in your R console: `learnr::run_tutorial("introduction", "stat500r")`.
